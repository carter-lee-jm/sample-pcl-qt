...
PCLViewer::loadFileButtonPressed()
{
	...
	QString filename = QFileDialog::getOpenFileName(this, tr("Open point cloud"), "/home/", tr("Point cloud data (*.pcd *.ply *.txt)"));
	...
	{
		PCL_ERROR("File chosen does not have a valid extension.\n");
		return;
	}
...
void PCLViewer::createDirectory()
{
	for (int i = 1; i <= 10; ++i)
	{
		std::ostringstream folderName;
		folderName << "test5566" << i;
		boost::filesystem::create_directory(folderName.str());
		boost::this_thread::sleep_for(boost::chrono::seconds(1));
		std::cout << "create " << folderName.str() << std::endl;
	}
}

void PCLViewer::runBoostTest()
{
	boost::thread thread([this] { createDirectory(); });
	thread.detach();
}
